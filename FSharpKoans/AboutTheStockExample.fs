...
module ``about the stock example`` =
    ...
    let getDateOfGreatesDifferenceBetweenOpeningAndClosingPrices (stockData: string list) =

        let getPrice price = Double.Parse(price, CultureInfo.InvariantCulture)

        let getDifferenceBetweenOpeningAndClosingPrice (stockEntry: string []) =
            { Date = stockEntry.[0]; PriceDifference = getPrice stockEntry.[4] - getPrice stockEntry.[1]; }
        
        let greatestDateAndPrice =
            stockData
                |> List.skip 1
                |> List.map (fun x -> x.Split [| ',' |])
                |> List.map getDifferenceBetweenOpeningAndClosingPrice
                |> List.maxBy (fun x -> x.PriceDifference)
        
        greatestDateAndPrice.Date
    ...
    let YouGotTheAnswerCorrect() =
        let result = getDateOfGreatesDifferenceBetweenOpeningAndClosingPrices stockData
